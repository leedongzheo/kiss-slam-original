...
# import sys
# from collections import deque zz
# import numpy as np
...
class RegistrationVisualizer(StubVisualizer):
    def __init__(self):
        ...
        # === [BỔ SUNG 1] BIẾN ĐIỀU KHIỂN BOUNDARY-ZOOM CHỮ NHẬT ===
        self.map_start_pose = None
        self.bound_x = 35.0  # Ngưỡng viền ngang (trái/phải)
        self.bound_y = 35.0 * (1080.0 / 1920.0) # Ngưỡng viền dọc ép theo tỷ lệ 16:9
        self.zoom_ratio = 1.35
    ...
    def _update_geometries(self, slam):
        ...
        # === [BỔ SUNG 2] LOGIC CHỤP GÓC NHÌN TÂM & ZOOM TRÀN MÀN HÌNH THEO TỶ LỆ CHỮ NHẬT ===
        current_xyz = current_node.endpose[:3, 3]
        if self.map_start_pose is None:
            self.map_start_pose = current_xyz.copy()

        if not self.follow_cam:
            # Tính khoảng cách độc lập theo 2 trục X và Y
            dx = abs(current_xyz[0] - self.map_start_pose[0])
            dy = abs(current_xyz[1] - self.map_start_pose[1])
            
            # Kích hoạt Zoom khi chạm viền ngang HOẶC chạm viền dọc
            if dx > self.bound_x or dy > self.bound_y:
                ctr = self.vis.get_view_control()
                ctr.set_zoom(self.zoom_ratio)
                
                # Nới rộng cả khung chữ nhật ra cho lần chạy tới
                self.bound_x *= self.zoom_ratio
                self.bound_y *= self.zoom_ratio
                print(f"[Camera] Chạm viền (dx={dx:.1f}m, dy={dy:.1f}m). Đã Zoom out x{self.zoom_ratio}!")
